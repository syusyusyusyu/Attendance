...
20. Q: 早退判定はどうする？
    A: 滞在時間が授業分数の80%未満（最低出席率）だと早退です。

21. Q: 出席ポリシーの主項目は？
    A: 遅刻/締切、レート制限、IP/ブラウザ制限、警告閾値です。
...
24. Q: レート制限は？
    A: 既定はクラス10回/分、学生6回/分です。

25. Q: IP制限は？
    A: 許可IPレンジで教室内のみ許可できます。

26. Q: ブラウザ制限は？
    A: 許可ブラウザキーワードで制限可能です。

27. Q: 位置情報必須とは？
    A: 位置情報が取得できない場合は出席登録できない設定です。

28. Q: 不正検知の例は？
    A: 失敗4回/2分、同一IP8回/1分、QR共有2人/2分などです。
...
53. Q: 位置情報は何に使う？
    A: QRスキャン時の教室内判定と不正抑止に使います。

54. Q: 操作申請の承認はどこで行う？
    A: 管理者の操作申請一覧で承認/却下します。
