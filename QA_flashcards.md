...
40. Q: QRカメラスキャンに非対応の端末はどうする？
    A: 手入力でQR文字列を登録できる。

41. Q: QR_TOKEN_SECRET を変更すると何が起きる？
    A: 既存のQRトークンは全て無効になる。

42. Q: 重複スキャン時の挙動は？
    A: 既に出席済みとして処理し、スキャンログに duplicate を残す。
...
81. Q: 遅刻判定や締切はどこで設定する？
    A: 教員の出席管理画面で出席ポリシーを設定できる。

82. Q: QRスキャンログは確認できる？
    A: `/scan-logs` で監査ログを確認できる。

83. Q: CSVインポートの必須列は？
    A: `日付/学生ID/出席状況` が必須で、`未入力` はスキップされる。

84. Q: クラス管理はどこから？
    A: 教員の「クラス管理」画面で作成・名簿インポート・履修管理ができる。

85. Q: 休講や補講は設定できる？
    A: クラス詳細の「特別日程」で日付単位に設定できる。

86. Q: 出席修正の理由は残る？
    A: 修正理由が必須で、出席変更ログに記録される。

87. Q: レポート機能はある？
    A: `/reports` で期間指定の出席集計を確認できる。

88. Q: 通知はどこで見る？
    A: `/notifications` で確認できる。

89. Q: QRスキャンのセキュリティは？
    A: IP/端末制限と分間スキャン上限を設定できる。

90. Q: QRスキャンはリアルタイム反映される？
    A: 教員の出席管理画面に即時反映される。
