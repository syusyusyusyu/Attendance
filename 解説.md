...
│                                                              │
│      def create                                              │
│        result = QrScanProcessor.new(...).process  # ③       │
│        if result.success?                                   │
...
| `QrScanProcessor` | スキャン処理の全体統括 | `#process` |
| `AttendanceFinalizer` | 締切後の自動欠席確定 | `#finalize` |
| `TermReportBuilder` | 期末レポート生成 | `#build` |
| `AttendanceStatsCalculator` | 出席統計計算 | `#calculate` |
...
> 4. 不正検知で、失敗多発やIP集中を検知して教員に通知（※通知機能は未実装）」

### Q5. 改善したい点は？

> 「現在は通知機能が未実装なので、メール・LINE・Push通知を実装したいです。  
