...
テーマ切替はStimulusの典型的な活用例です。UIの状態をHTML属性で管理し、CSSカスタムプロパティで配色を一括切替します。
...
```css
/* CSS変数で配色を一括管理 */
:root { --color-background: #FAFAFA; --color-surface: #FFFFFF; }
html[data-theme="dark"] { --color-background: #111118; --color-surface: #1A1A26; }
```

**ポイント**: CSS変数を`html[data-theme="dark"]`で上書きするだけで、全コンポーネントがダークモードに対応します。個別のクラス切替は不要です。
...
```ruby
# settings カラムに通知設定やテーマを保存
class User < ApplicationRecord
  # settings: { email_notify: true, line_notify: false, theme: "dark", ... }
