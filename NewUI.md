...
- 主要機能：
  ...
  - 通知（メール/LINE/Push）⚠️未実装（UI上は「未実装」表示を明確に。勝手に実装済みに見せない）
...
- 不正検知：失敗多発/IP集中/トークン共有検知 → 教員通知（⚠️通知は未実装）
...
## 7.6 学生：通知（⚠️未実装）
- UI上に「⚠️ 未実装」または同等の日本語注記を明確に表示し、実装済みに見せない
- 既存に通知一覧があるなら現状維持。無いなら“プレースホルダー”として表示のみ。

## 7.7 学生：プロフィール
- 基本情報（氏名/メール/学生ID/パスワード）表示/編集
- 通知設定（メール/LINE/Push）⚠️未実装は明示
- Push購読/解除ボタン（未実装ならUIは無効化＋注記）
...
- 機能階層図（認証/ダッシュボード/QR出席/出席管理/監査ログ/クラス管理/レポート/通知未実装/管理者機能）
