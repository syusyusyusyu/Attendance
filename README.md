...
展示に向けて「AdminKit風」のモダンなUIへ刷新しました。機能や画面構成は維持しつつ、見た目と操作性を改善しています。

| 項目 | 旧UI（UI.md） | 新UI（NewUI.md） |
|:---|:---|:---|
| **テーマ** | Google系配色（緑/青/赤/黄） | AdminKit風（濃紺サイドバー＋青アクセント） |
| **サイドバー** | ライト背景、緑ハイライト | ダーク背景（#222e3c）、青ハイライト＋左インジケータ |
| **カード** | 白背景、薄い影 | 白背景、角丸10-14px、控えめな影、余白広め |
| **KPIカード** | - | タイトル＋大数値＋右上アイコンバッジ |
| **テーブル** | 横スクロール対応 | ヘッダー薄グレー、行間ゆったり、ピルバッジ |
| **フォント** | Roboto / Noto Sans JP | Inter / Noto Sans JP |
| **学生SP** | サイドバー（ドロワー式） | 下部ナビ（Bottom Nav）固定 |
| **全体印象** | 機能的な管理画面 | SaaS風ダッシュボード、UI密度低めで読みやすい |

> **変更方針**: 日本語の文言（メニュー名、ボタン、ラベル等）は一切変更せず、視覚的なスタイルのみを刷新。
