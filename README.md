...
**初期**: Google系配色（緑/青/赤/黄のマテリアル調）
**Phase 1**: AdminKit風（濃紺サイドバー＋青アクセント）
**Phase 2**: SaaSモダンミニマル（Linear/Notion/Vercel風）

| 項目 | 初期（Google系） | Phase 1（AdminKit風） | Phase 2（SaaSミニマル） |
|:---|:---|:---|:---|
| **テーマ** | Google配色（#4285F4 / #34A853 / #EA4335） | 濃紺サイドバー＋青アクセント（#3B7DDD） | ミュートインディゴ（#5B5BD6）、ニュートラル背景 |
| **サイドバー** | ライト背景、緑ハイライト | ダーク背景（#222e3c）、グラデーション＋重シャドウ | ライト白背景、右ボーダーのみ、幅15rem |
| **トップバー** | 白背景、薄い下線 | ガラスモーフィズム（blur＋半透明）、高さ4rem | フラット白＋下ボーダーのみ、高さ3rem |
| **カード** | 白背景、薄い影、角丸4px | 角丸14px、shadow-sm、hover時translateY | 角丸8px、shadow: none、hover時borderのみ |
| **KPIカード** | なし（テキスト表示のみ） | 48px円アイコン、text-3xl bold、padding 20px | 32px角丸アイコン、text-2xl semibold、padding 16px |
| **テーブル** | 横スクロール対応、最小スタイル | ヘッダー薄グレー背景、uppercase、letter-spacing | ヘッダー透明背景、通常テキスト、コンパクトpadding |
| **ボタン** | Google Material風、shadow-google | px-5 py-2.5、min-h 44px、角丸14px、ボタン影あり | px-3 py-1.5、min-h 36px、角丸6px（SP時44px維持） |
| **バッジ** | 角丸4px、原色背景＋白文字 | pill型（9999px）、淡色背景＋濃い文字 | 角丸6px、px-2 py-0.5 |
| **シャドウ** | Google shadow（0 1px 2px + 0 1px 3px） | sm/md/lg の3段階、ボタンにも影 | ほぼ消去（sm: none）、ボーダーで分離 |
| **角丸** | 4px統一 | 8/12/14/16px | 4/6/8/10px |
| **フォント** | Roboto / Noto Sans JP | Roboto / Noto Sans JP | Inter / Noto Sans JP |
| **ボトムナビ** | なし（サイドバードロワー式） | glass/blur/shadow、角丸あり | フラット白＋上ボーダーのみ、角丸なし |
| **トースト** | 原色背景＋白文字 | 原色背景＋白文字 | ライト背景＋ボーダー＋ダークテキスト |
| **オーバーレイ** | rgba(0,0,0,0.5) | rgba(0,0,0,0.5)＋blur | rgba(0,0,0,0.3)、blurなし |
| **全体印象** | 機能的なGoogle管理画面 | SaaS風ダッシュボード、UI密度低め | ミニマルSaaS、フラット＋ボーダー、情報密度高め |
