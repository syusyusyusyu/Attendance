...
- Role/Permission/RolePermission
  - 必須項目と一意制約
- OperationRequest
  - kind/status必須、payloadはjsonbで保持
- AuditSavedSearch
  - name/scope必須、defaultは1件のみ
- ApiKey
  - token_digest一意、revoked_atで無効化
- Device
  - user_id/device_id一意、承認フラグで制御
- SsoProvider/SsoIdentity
  - name/strategy/uid必須、uid一意
...
- AttendanceFinalizer
  - 授業回の締切後に欠席を自動確定できる
- RosterCsvImporter
  - 名簿CSVから管理者のみ新規作成、教員は既存学生の更新/履修登録ができる
- QrFraudDetector
  - 不正検知で通知を作成できる
- TermReportBuilder
  - 期末レポート用の集計結果を生成できる
- OperationRequestProcessor
  - 承認済みの申請を実行しログ/通知が作成される
