...
- データ永続化: PostgreSQL
- 主要URL: `/login`, `/`, `/scan`, `/generate-qr`, `/history`, `/attendance`, `/attendance_requests`, `/attendance-logs`, `/scan-logs`, `/reports`, `/notifications`, `/school_classes`, `/profile`, `/admin`, `/admin/users`, `/admin/roles`, `/admin/operation_requests`, `/admin/devices`, `/admin/api_keys`, `/terms`, `/privacy`, `/data-policy`
...
flowchart LR
  ...
  W --> M[Model<br/>User/Role/Permission/RolePermission/SchoolClass/Enrollment/ClassSession/AttendanceRecord/AttendanceRequest/AttendanceChange/AttendancePolicy/AttendanceStatus/QrSession/QrScanEvent/Notification/OperationRequest/AuditSavedSearch/ApiKey/Device]
...
flowchart TB
  ...
  AC --> AC24[api/base_controller]
  AC --> AC25[api/v1/classes_controller]
  A --> AM[models/]
  AM --> AM1[user]
  AM --> AM2[school_class]
  AM --> AM3[enrollment]
...
- 主要処理: 失敗理由/結果/端末情報を `QrScanEvent` に記録
...
- `test/services/qr_scan_processor_test.rb`
  ...
  - rate_limit/ip_blocked/device_blocked/require_device が拒否される。
...
- `test/integration/admin_flow_test.rb`（管理/権限/API/端末/操作申請）
...
### 9.6 E2E想定動作一覧（ブラウザ）
- 認証: 未ログインで保護ページへアクセスするとログイン画面へリダイレクトされる
- 認証: アカウントタイプ切り替えでボタン色とロール値が更新される
- 認証: パスワードの表示/非表示で入力欄の表示が切り替わる
- 認証: デモ用ログインを押すとメール/パスワードが自動入力される
- 認証: 正しい情報で「ログイン」を押すとダッシュボードへ遷移する
- 認証: 保護ページへの直前アクセスがあればログイン後に元のページへ戻る
- 認証: 誤った情報で「ログイン」を押すとエラーメッセージが表示される
- 認証: ログイン失敗が一定回数を超えるとロック案内が表示される
- 認証: ログアウトでセッションが破棄されログイン画面へ戻る
- 権限: 権限のない画面へアクセスすると「アクセス権限がありません」が表示される
- ダッシュボード: ロールに応じたカード導線が表示される
- ダッシュボード: 学生の初回ガイドが初回のみ表示される
- ダッシュボード: 担当/履修クラスがない場合は空表示が出る
- ダッシュボード: クラス一覧から「出席確認/出席登録/履歴」に遷移できる
- ダッシュボード: 教員は本日の出席サマリーが表示される
- ダッシュボード: 教員は要注意者/申請待ち/通知のカードが表示される
- ダッシュボード: 学生は30日出席傾向と注意アラートが表示される
- ダッシュボード: 未読通知の件数バッジが表示される
- 教員: QR生成画面でクラス未選択時は案内が表示される
- 教員: クラス選択でQR画像/有効期限/クラス情報が表示される
- 教員: 授業回ステータスバッジが表示される
- 教員: QRを再発行すると以前のQRは無効になり最新のみ有効になる
- 教員: 残り時間が自動カウントダウンされる
- 教員: QRのダウンロードボタンでPNGが保存できる
- 学生: QRスキャン画面でカメラ枠と動画領域が表示される
- 学生: 「カメラを開始」でステータスが更新される
- 学生: 「停止」でカメラが停止される
- 学生: 「手入力」で入力フォームが表示される
- 学生: カメラ未対応/権限拒否時は手入力案内が表示される
- 学生: 有効なQRで「出席を記録しました」が表示される
- 学生: 同一授業回の再スキャンで「退室を記録しました」が表示される
- 学生: 退室済みの再スキャンで「退室済み」表示になる
- 学生: 期限切れ/失効/無効のQRは理由付きのエラーが表示される
- 学生: 日付不一致/未履修/休講/予定なしは理由付きエラーになる
- 学生: 出席確定済み/手動修正済みはスキャン不可のエラーになる
- 学生: レート制限/端末制限/IP制限に該当するとエラーになる
- 学生: スキャン結果はログに記録され出席一覧へ反映される
- 学生: 出席履歴で日付を選ぶと当日の出席情報が表示される
- 学生: 出席履歴に申請ステータスと理由が表示される
- 学生: 出席履歴CSV/PDFを期間指定で出力できる
- 学生: 日付形式が不正な場合はエラーが表示される
- 学生: 出席申請でクラス/日付/種別/理由の必須チェックが効く
- 学生: 同日同クラスの未処理申請があると重複エラーになる
- 学生: 申請後に「送信しました」メッセージが表示される
- 教員: 出席申請一覧でクラス/状態のフィルタが効く
- 教員: 出席申請を承認/却下するとステータスが更新される
- 教員: 一括承認/却下で処理件数が反映される
- 教員: 出席管理でクラス/日付を選択して一覧が表示される
- 教員: 理由未入力の修正はエラーになる
- 教員: 出席修正は操作申請として登録される
- 教員: CSVインポートは操作申請として登録される
- 教員: 出席確定/解除は操作申請として登録される
- 管理者: 出席修正は即時反映されログが記録される
- 管理者: CSVインポートが即時反映され結果が表示される
- 管理者: 出席確定は締切後のみ実行される
- 管理者: 出席確定/解除が即時反映される
- 教員/管理者: CSVエクスポートで期間指定のCSVが出力される
- 教員/管理者: 出席確定済み授業は修正不可の警告が出る
- 教員/管理者: スキャンログでクラス/日付/結果/判定の絞り込みができる
- 教員/管理者: スキャンログをCSVで出力できる
- 教員/管理者: 出席変更ログで期間/種別/ステータス/検索の絞り込みができる
- 教員/管理者: 条件保存/既定条件が適用される
- 教員/管理者: 出席変更ログをCSVで出力できる
- 教員/管理者: レポートで期間指定の集計が表示される
- 教員/管理者: クラス選択で詳細/推移/要注意者/理由分布が表示される
- 教員/管理者: 期末レポートCSV/PDFを出力できる
- 教員/管理者: クラス未選択のCSV/PDF出力はエラーになる
- 通知: 通知一覧が表示され未読/既読が判別できる
- 通知: 一括既読で未読バッジが更新される
- プロフィール: 基本情報の更新が保存される
- プロフィール: パスワード変更が保存される
- プロフィール: 端末一覧と承認申請ボタンが表示される
- プロフィール: 通知チャネル設定が保存される
- 管理者: 端末管理で一覧/状態フィルタが表示される
- 管理者: 端末の承認/解除が反映される
- 管理者: 管理ダッシュボードが表示される
- 管理者: ユーザー管理の一覧/作成/編集/削除ができる
- 管理者: 役割/権限の設定が保存される
- 管理者: 操作申請の承認/却下が実行できる
- 管理者: APIキーの作成/失効が実行できる
