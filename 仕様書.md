...
- 位置情報（QRスキャン時）:
  - 位置情報取得の必須化: `require_location`
  - 位置情報精度の上限: `geo_accuracy_max_m`（m）
  - ジオフェンス: `geo_fence_enabled` が有効な場合に `geo_center_lat` / `geo_center_lng` を中心に `geo_radius_m`（m）で範囲判定
  - 教室住所: 管理者が `geo_address` を設定（表示/管理用）
- レート制限（QRスキャン）:
...
- QRスキャンは位置情報取得が必須で、精度上限/ジオフェンスで判定する
