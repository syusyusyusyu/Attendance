...
- `test/system/e2e_smoke_test.rb`
  - 学生ロールに `qr.scan` 権限を付与する前提を作成。
  - PC表示: 管理者でログイン → `/generate-qr` を開き、QR画像要素が表示される。
  - モバイル表示: 学生でログイン → `/scan` を開き、カメラ枠と動画要素が表示される。
  - 参照: 9.5
...
- PC表示(1400x1400)
  - 管理者でログイン → `/generate-qr` を開き、QR画像が表示される
- モバイル表示(390x844)
  - 学生でログイン → `/scan` を開き、カメラ枠と動画要素が表示される
- 参照: `test/system/e2e_smoke_test.rb`

### 9.6 E2E想定動作一覧（ブラウザ）
- 認証: 未ログイン時は `/login` へリダイレクトされる
- 認証: 正しい認証情報でログインできる
- 認証: 誤った認証情報はエラーメッセージで拒否される
- 教員: `/generate-qr` でQRが表示され、発行期限が表示される
- 教員: QR再発行で以前のセッションが失効し、最新QRのみ有効になる
- 学生: `/scan` でカメラ枠と動画が表示される
- 学生: カメラ許可拒否時は手入力にフォールバックできる
- 学生: 有効トークンで出席が登録される
- 学生: 期限切れ/失効/無効トークンは理由付きでエラーになる
- 学生: 同一授業回の重複スキャンは「既に出席済み」と表示される
- 学生: `/history` で当日の出席履歴が表示される
- 教員: `/attendance` でクラス/日付の出席一覧が表示される
- 教員: 出席更新で変更ログが作成される（管理者は即時反映）
- 教員: `/attendance_requests` で申請の承認/却下ができる
- 教員: `/scan-logs` `/attendance-logs` のフィルタ結果が表示される
- 管理者: 操作申請の承認で出席修正が反映される
- 管理者: `/admin/users` `/admin/roles` で管理画面が表示される
