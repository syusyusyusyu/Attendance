...
### Q: 管理者画面で何ができますか？
A: ユーザー/権限/監査/操作申請/端末承認/SSO/APIキーを一元管理できます。

### Q: 操作申請とは？
A: 出席修正/確定/解除/CSV反映などの重要操作を管理者承認で実行する仕組みです。

### Q: 公認端末の運用は？
A: 承認済み端末のみQRスキャンを許可する運用が可能です。

### Q: API連携は？
A: APIキーとスコープ認可でクラス/名簿/出席の取得ができます。

### Q: SSOは使えますか？
A: OIDC/SAMLの設定値を管理画面で保持でき、実運用の統合に拡張できます。
...
### Q: 遅刻や締切の判定はどう決める？
A: 大阪情報コンピュータ専門学校の基準に固定しています。出席QRのスキャンは出席、授業開始20分以降は欠席です。
...
### Q: 期末レポート(PDF/CSV)は出力できる？
A: `/reports` でクラスと期間を選択してPDF/CSVを出力できます。

### Q: 不正検知アラートは？
A: 失敗スキャン多発、同一IP集中、IP/端末制限、QR共有の検知で通知されます。
