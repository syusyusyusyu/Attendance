...
### Q: 管理者画面で何ができますか？
A: ユーザー/権限/監査/操作申請を一元管理できます。

### Q: 操作申請とは？
A: 出席修正/確定/解除/CSV反映などの重要操作を管理者承認で実行する仕組みです。

### Q: API連携は？
A: 現状は提供していません。必要になれば将来追加します。
...
### Q: QRスキャンの監査ログは見られる？
A: 教員は `/scan-logs` でスキャン結果と出席判定、IP/ブラウザ情報を確認できます。
...
### Q: QRスキャンのセキュリティは？
A: 分間スキャン上限、許可IP/ブラウザキーワードの制限を設定できます。
...
### Q: 不正検知アラートは？
A: 失敗スキャン多発、同一IP集中、IP/ブラウザ制限、QR共有の検知で通知されます。
